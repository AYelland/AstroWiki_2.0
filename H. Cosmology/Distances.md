...
> 
> $$\rd x = c \; \rd t \quad , \quad \rd x  = a(t) \; \rd \chi \hWhere \begin{aligned} \rd x &\equiv d_{p} \equiv \text{proper distance} \\ \rd \chi &\equiv d_{C} \equiv \text{comoving distance} \end{aligned}$$

## Comoving Distance

The **comoving distance** ($\rd \chi \equiv d_{C}$) is the distance between two points in spacetime after we have factored out the expansion of universe.
...
$$\rd x = a(t) \, \rd \chi \hspace{1cm} \Longleftrightarrow \hspace{1cm} d_{p} = a(t) \, d_{C}$$
...
$$d_{C} = \int_{t_{\rm emt}}^{t_{\rm obs}} \frac{c \; \rd t}{a(t)} \hspace{1.5cm} d_{C} = d_{H} \int_{a_{\rm emt}}^{a_{\rm obs}} \frac{\rd a}{a^{2} \, E(a)} \hspace{1cm} \underbrace{d_{C} = d_{H} \int_{z_{\rm obs}}^{z_{\rm emt}} \frac{\rd z}{E(z)}}_{\text{note the switched integration bounds}}$$
...
> 
> $$d_{C} \equiv r = \int_{t_{\rm emt}}^{t_{\rm obs}} \frac{c \; \rd t}{a(t)}$$
> 
...
> 
> $$H = \frac{\dot{a}}{a} = \td{a}{t} \frac{1}{a} \hRightarrow \rd t= \frac{1}{H} \frac{\rd a}{a}$$
> $$r = \int_{t_{\rm emt}}^{t_{\rm obs}} \frac{c \; \rd t}{a(t)} = \int_{a_{\rm emt}}^{a_{\rm obs}} \frac{c \; \rd a}{a^{2} H(a)} = \frac{c}{H_{0}} \int_{a_{\rm emt}}^{a_{\rm obs}} \frac{\rd a}{a^{2} E(a)}$$
> 
...
> 
> $$a(z) = \frac{1}{1+z} \hRightarrow \rd a = -\frac{\rd z}{(1+z)^{2}} = -a^{2} \; \rd z$$
> $$r = \frac{c}{H_{0}} \int_{t_{\rm emt}}^{t_{\rm obs}} \frac{\rd a}{a^{2} E(a)} = - \frac{c}{H_{0}} \int_{z_{\rm emt}}^{z_{\rm obs}} \frac{\rd z}{E(z)} = \frac{c}{H_{0}} \int_{z_{\rm obs}}^{z_{\rm emt}} \frac{\rd z}{E(z)}$$
> 
...
> 
> $$\boxed{d_{C} = \int_{t_{\rm emt}}^{t_{\rm obs}} \frac{c \; \rd t}{a(t)}} \hspace{1.5cm} \boxed{d_{C} = d_{H} \int_{a_{\rm emt}}^{a_{\rm obs}} \frac{\rd a}{a^{2} \, E(a)}} \hspace{1cm} \underbrace{\boxed{d_{C} = d_{H} \int_{z_{\rm obs}}^{z_{\rm emt}} \frac{\rd z}{E(z)}}}_{\text{note the switched integration bounds}}$$
...
The **proper distance** ($\rd x \equiv d_{p}$) is the *instantaneous* distance between points in an expanding universe, which changes with the [[Hubble#Hubble Law|Hubble flow]]. 
...
$$d_{A} = \frac{\ell}{\delta \theta} = \frac{a(z) \, f_{k}(\chi) \, \delta\theta}{\delta \theta} = \frac{f_{k}(\chi)}{1+z} \hWhere \chi = d_{C}$$
$$f_{k}(\chi) = (1+z) \; d_{A}$$
In a flat universe ($k=0$), the angular diameter distance is simply given in terms of the [[#Comoving Distance|comoving distance]].

$$f_{k}(\chi) = \chi \equiv d_{C} \hRightarrow d_{A} = \frac{d_{C}}{1+z}$$
...
$$L = F \cdot A = F \left( 4 \pi R^{2} \right) \hRightarrow d_{L} \equiv R = \sqrt{\frac{L}{4 \pi F}}$$

In terms of the [[#Comoving Distance|comoving distance]] and [[#Angular Diameter Distance|angular diameter distance]]...

$$d_{L} = (1+z) \; f_{k}(\chi) = (1+z)^{2} \; d_{A} \hWhere \chi \equiv d_{C}$$
...
> 
> $$L_{\rm obs} = \frac{h \nu_{\rm obs}}{\delta t_{\rm obs}} = L_{\rm emt} \fpar{a_{\rm emt}}{a_{\rm obs}}^{2} = L_{\rm emt} (1+z)^{2}$$
> 
